...
$i = 0;
for ($k = 0; $k < sizeof($nameList); $k++) {
    if($db->updateData($type,$artefactName,$nameList[$k],$dataList[$k])) {
        $i++;
    }
}

if($i == sizeof($nameList)) {
    echo "Success";
}
...
//
//
//        $db->updateData($type,$artefactCode,$artefactCode,$dataList[$index]);
//        $db->setQuery($sql2);
//
//
//        if ($result)
